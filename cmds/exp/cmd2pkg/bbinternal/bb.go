...
func NewPackage(name string, p *packages.Package) *Package {
	...
	pp.mainFuncName = pp.newFunctionName("Main")

	// This Init will hold calls to all other InitXs.
	pp.init = &ast.FuncDecl{
		Name: ast.NewIdent(pp.newFunctionName("Init")),
		Type: &ast.FuncType{
...
// github.com/0magnet/u-root/vendor/golang.org/x/sys/unix. importName returns
...
func (p *Package) PackageName() string {
	return pnameRegex.ReplaceAllString(p.Name, "")
}
...
// Rewrite rewrites p into destDir as a package, rewriting its init and main
// functions.
